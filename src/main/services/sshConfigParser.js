/**
 * SSH Config Parser Service for DevsFTP
 * Parses ~/.ssh/config and /etc/ssh/ssh_config into DevsFTP profile objects.
 */
...
function parseSSHConfigText(configText) {
  ...
  lines.forEach(line => {
    ...
    // Directives can be space or equals separated e.g. "HostName 10.0.0.1" or "HostName=10.0.0.1"
    ...
    if (key === 'host') {
      ...
      if (key === 'hostname') {
        ...
        // Use first identity file if multiple
